...
public class UserTaxDaoImpl implements UserTaxDao {
	@Override
	public List<UserTax> findUserTaxesByMonth(int month, int year, int command) { // tìm thuế theo tháng hoặc 3 người
																					// nộp thuế cao nhất
		...
		try (Connection conn = DBConnection.getCon();
				...
				ResultSet rs = stmt.executeQuery("SELECT * FROM user_tax WHERE thoi_gian_chiu_thue >= '" + min
						+ "' AND thoi_gian_chiu_thue < '" + max + "' ORDER BY thue_phai_nop DESC " + addition);) {
	...
	public List<Integer> getYears() {
		...
		try (Connection conn = DBConnection.getCon();
				Statement stmt = conn.createStatement();
				ResultSet rs = stmt.executeQuery("SELECT DISTINCT YEAR(thoi_gian_chiu_thue) as year FROM user_tax ORDER BY year deSC");) {
